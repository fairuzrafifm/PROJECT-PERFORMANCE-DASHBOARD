...
function renderDailyReport(){
  ...
  leafNodes.forEach(node=>{
    ...
    if(weekNum&&node.weeklyPlan&&node.weeklyPlan[weekNum]){
      ...
      // Fallback: cari minggu terakhir yang ada di weeklyPlan sebelum weekNum
      // (bukan node.cumPlan yang nilainya final=100, melainkan plan aktual di minggu terdekat)
      const _wp=node.weeklyPlan||{};
      const _prevWeeks=Object.keys(_wp).map(Number).filter(k=>k>0&&k<weekNum).sort((a,b)=>b-a);
      if(_prevWeeks.length){
        const _lastW=_prevWeeks[0];
        cumPlan+=(bobot/100)*((+_wp[_lastW].cumPlan||+_wp[_lastW].wPlan||0)/100);
      }
      // Jika tidak ada weeklyPlan sama sekali → kontribusi 0 (item belum dijadwalkan)
    }
  });
...
function renderDrMainTable(projId,date){
  ...
  const TH = (label,align,w) =>
    `<th style="padding:9px ${align==='right'?'14px':'10px'} 9px 10px;text-align:${align||'left'};font-size:9px;text-transform:uppercase;letter-spacing:.5px;color:var(--mt);font-weight:700;white-space:nowrap;background:var(--sf2);position:sticky;top:0;z-index:2;box-shadow:inset 0 -2px 0 var(--bd),0 4px 7px -3px rgba(0,0,0,.4);border-bottom:2px solid var(--bd)${w?';width:'+w:''}">` + label + '</th>';
...
function saveDrQtySetup(){
  ...
  dirty();cm('drQtySetup');renderDailyReport();toast('✓ Setup qty disimpan')
}
...
function delDailyLog(nodeId,date){
  ...
  showConfirm(`Hapus log ${date}?`,()=>{
    ...
    dirty();renderDailyReport();renderDailyHistoryTable(node.projId)
    toast('Log dihapus','warn');
  });
}


// ── DAILY REPORT LOCK (merged from patch3) ───────────────────────────────────

var _origSaveDrInput = (typeof saveDrInput === 'function') ? saveDrInput : null;
if (_origSaveDrInput) {
  saveDrInput = async function() {
    var projId = document.getElementById('drProjSel') && document.getElementById('drProjSel').value;
    if (projId && typeof _checkAndWarnLock === 'function') {
      var locked = await _checkAndWarnLock('daily', projId);
      if (locked) return;
    }
    if (projId && typeof _acquireLock === 'function') await _acquireLock('daily', projId);
    try {
      return await _origSaveDrInput.apply(this, arguments);
    } finally {
      if (projId && typeof _releaseLock === 'function') _releaseLock('daily', projId);
    }
  };
}
