...
(function () {
  ...
  function _enhance(id) {
    ...
    q.addEventListener('keydown', function (e) {
      if (e.key === 'Escape') { e.stopPropagation(); _close(pop); try { btn.focus(); } catch (er) {} }
  ...
  window.addEventListener('scroll', function (e) {
    // Jangan tutup bila yang di-scroll adalah daftar DI DALAM popup (mis. .ssel-list).
    if (_open && e.target && _open.contains && _open.contains(e.target)) return;
    _closeAll();
  }, true);
