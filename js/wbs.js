...
function _wbsPlanRow(node,ci,si,weeks,curWeek,proj,ii){
  ...
  weeks.forEach(w=>{
    ...
    const _rb=isNext?'1px solid rgba(249,115,22,.5)':'1px solid var(--bd)';
    const _rbg=isNext?'rgba(249,115,22,.06)':'transparent';
...
function syncWbsToSCurve(projId){
  ...
  [...weekSet].sort((a,b)=>a-b).forEach(week=>{
    ...
    leafNodes.forEach(node=>{
      ...
      // cAct: gunakan weeklyData.cAct jika ada, fallback ke cumActual hanya di minggu terakhir aktual
      ...
      // Minggu terakhir yang punya dailyLog aktual
      const lastDailyWeek=Math.max(0,...(node.dailyLogs||[]).map(l=>+l.week||0));
      // Terapkan cumActual HANYA ke minggu terakhir aktual (bukan semua minggu sesudahnya)
      const isExactLastActualWeek=lastDailyWeek>0&&week===lastDailyWeek;
      if(maxWeekWithAct>0){
        cActContrib+=bobot*(+wd[maxWeekWithAct].cAct||0)/100;
      }else if(isExactLastActualWeek&&(+node.cumActual||0)>0){
...
function _renderWbsSCurveNow(projId){
  ...
  if($('wbsScKpi'))$('wbsScKpi').innerHTML=`
    <div style="background:var(--sf2);border-radius:8px;padding:8px 10px;text-align:center"><div style="font-size:9px;color:var(--mt);text-transform:uppercase;letter-spacing:1px;margin-bottom:2px">Cum. Plan${_wlbl}</div><div style="font-family:var(--fd);font-size:20px;color:var(--gn)">${cPlan.toFixed(1)}%</div></div>
