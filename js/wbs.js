...
function renderWBS(){
  // PERF Tahap 2a: bind listener scroll SEKALI -> matikan hover-repaint saat scroll
  (function(){var el=$('wbsTable');if(el&&!el._scrollBound){el._scrollBound=1;var _t;el.addEventListener('scroll',function(){if(!el.classList.contains('is-scrolling'))el.classList.add('is-scrolling');clearTimeout(_t);_t=setTimeout(function(){el.classList.remove('is-scrolling');},90);},{passive:true});}})();
  ...
  // ── INDEX sekali: rollup & lookup anak jadi O(n), bukan O(n^2)/O(n^3) ──
  // Output identik; menghindari all.filter/all.some berulang di dalam loop.
  const _childrenBy=new Map();
  all.forEach(w=>{const k=w.parentId;let _a=_childrenBy.get(k);if(!_a){_a=[];_childrenBy.set(k,_a);}_a.push(w);});
  const _kidsOf=id=>_childrenBy.get(id)||[];
  const _subsOf=id=>_kidsOf(id).filter(w=>w.type==='subcat');
  const _itemsOf=id=>_kidsOf(id).filter(w=>w.type==='item');
  // Auto-sum bobot ke atas
  all.forEach(w=>{if(w.type==='subcat'){const cb=_itemsOf(w.id).reduce((s,x)=>s+(+x.bobot||0),0);if(cb>0)w._ab=cb;}});
  cats.forEach(cat=>{const sb=_subsOf(cat.id).reduce((s,sub)=>s+_itemsOf(sub.id).reduce((ss,x)=>ss+(+x.bobot||0),0),0);if(sb>0)cat._ab=sb;});
  ...
  const subcatLeaf=all.filter(w=>w.type==='subcat'&&!_itemsOf(w.id).length&&+w.bobot>0);
  ...
  if(!cats.length){
    ...
    cats.forEach((cat,ci)=>{
      ...
      const _dk=_kidsOf(cat.id);let catChildren=_dk.slice();_dk.forEach(s=>{catChildren=catChildren.concat(_kidsOf(s.id));});
      ...
      _subsOf(cat.id).sort((a,b)=>a.order-b.order).forEach((sub,si)=>{
        const sb=sub._ab||+sub.bobot||0;
        const subItems=_itemsOf(sub.id).sort((a,b)=>a.order-b.order);
        const isLeaf=subItems.length===0;
        const effectiveSb=isLeaf?+sub.bobot||0:subItems.reduce((s,x)=>s+(+x.bobot||0),0);
        // % Selesai subcat = rata-rata BERBOBOT: Σ(bobot item × %progress) ÷ Σ(bobot item) -> selalu <= 100%
        const subPct=isLeaf
          ?+(sub.cumActual||0)
          :(effectiveSb>0?subItems.reduce((s,x)=>s+(+x.bobot||0)*(+x.cumActual||0),0)/effectiveSb:0);
