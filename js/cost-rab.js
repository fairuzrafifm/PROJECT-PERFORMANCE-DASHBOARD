...
function saveRabKat(){
  ...
  if(typeof canEditProj==='function'&&!canEditProj(projId)){toast('Tidak punya akses edit ke proyek ini','error');return;}
  if(editRabCatId){
...
function delRabKat(){
  ...
  if(typeof canEditProj==='function'&&k&&!canEditProj(k.projId)){toast('Tidak punya akses edit ke proyek ini','error');return;}
  ...
  dirty();cm('addRabKat');renderRab();toast('Kategori dihapus','warn')
}
...
function delRabItem(){
  if(!editRabId)return;
  const _itDel=RAB.find(r=>r.id===editRabId);
  if(typeof canEditProj==='function'&&_itDel&&!canEditProj(_itDel.projId)){toast('Tidak punya akses edit ke proyek ini','error');return;}
  if(!confirm('Hapus item RAB ini?'))return;
  RAB=RAB.filter(r=>r.id!==editRabId);
  dirty();cm('addRabItem');renderRab();toast('Item RAB dihapus','warn')
}
...
function openCloneRabModal(){
  ...
  if(typeof canEditProj==='function'&&!canEditProj(destProjId)){toast('Tidak punya akses edit ke proyek tujuan ini','error');return;}
...
function executeCloneRab(){
  ...
  if(typeof canEditProj==='function'&&!canEditProj(destId)){toast('Tidak punya akses edit ke proyek tujuan ini','error');return;}
