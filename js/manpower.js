...
function _doDelHist(projId,idx){
  ...
  dirty();renderDetail(projId);toast('Entri dihapus','warn')
}
function saveHistEntry(){
  ...
  dirty();cm('editHist');renderDetail(projId);render();toast('Riwayat diupdate ✓')
}
...
function saveIss(){
  ...
  dirty();cm('addIssue');renderIssues();$('pk5').textContent=ISS.filter(i=>i.status!=='Closed').length
}

function delIss(){showConfirm('Hapus issue ini?',()=>{ISS=ISS.filter(i=>String(i.id)!==String(editIssId));dirty();cm('addIssue');renderIssues();toast('Issue dihapus','warn')});}
