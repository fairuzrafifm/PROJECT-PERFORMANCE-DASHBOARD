...
function renderGantt(){
  ...
  cats.forEach((cat,ci)=>{
    ...
    subcats.forEach((sub,si)=>{
      ...
      leftRows+='<div class="gantt-lrow gl-sub gc2_'+cat.id+'" style="padding-left:22px;'+(isLeaf?'':'cursor:pointer')+'"'+(isLeaf?'':' onclick="toggleGanttCat2(\'gi2_'+sub.id+'\')"')+'>'+(!isLeaf?'<span id="gi2icon_'+sub.id+'" style="font-size:9px;flex-shrink:0;opacity:.5">&#x25BC;</span>':'<span style="width:10px;flex-shrink:0"></span>')+'<span style="flex:1;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">'+(ci+1)+'.'+(si+1)+' '+safeStr(sub.name)+'</span>'+(subPct>0?'<span style="font-size:9px;color:var(--or);flex-shrink:0;font-family:var(--fm);margin-right:2px">'+subPct.toFixed(1)+'%</span>':'')+'<span class="gantt-dcol" style="display:flex;gap:2px;flex-shrink:0"><span class="gantt-date-val" style="width:62px">'+fmtGD(subSD)+'</span><span class="gantt-date-val" style="width:62px">'+fmtGD(subFD)+'</span></span></div>';
...
// (_ganttBar & _ganttActualBar dihapus — tak terpakai; renderGantt memakai mkBar/mkAct lokal)
...
// (toggleGanttCat versi lama dihapus — renderGantt memakai toggleGanttCat2)
