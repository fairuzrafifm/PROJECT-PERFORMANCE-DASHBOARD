...
function renderProjStatusCards(){
  ...
  el.innerHTML=filtered.map(p=>{
    ...
    const etaClr=p.status==='Critical'?'var(--rd)':vReal<0?'var(--yw)':'var(--gn)';
    ...
    return `<div class="psc ${scls}" onclick="selProj('${p.id}');sw('projects',document.querySelectorAll('.tab')[1])">
      ...
      <div class="psc-bar-wrap">
        <div class="psc-bar-lbl">
          <span>Plan${curWeekNum?` <span style="font-size:8px;background:rgba(59,130,246,.12);color:var(--bl);border:1px solid rgba(59,130,246,.25);border-radius:8px;padding:0 5px;font-family:var(--fm)">W${String(curWeekNum).padStart(2,'0')}</span>`:''}
          </span>
          <span style="color:var(--bl)">${curWeekPlan.toFixed(1)}%</span>
        </div>
        <div class="psc-bar"><div class="psc-bar-fill" style="width:0%;background:rgba(59,130,246,.45)" data-w="${curWeekPlan}"></div></div>
      </div>
      ${(()=>{
...
function renderDetail(id){
  ...
  $('detail').innerHTML=`<div class="fade">
    <div class="dtop" style="margin-bottom:11px">
      ...
      ${_canEdit ? `<div style="display:flex;gap:6px"><button class="btn btn-sm" onclick="openModal('editProj','${p.id}')"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="vertical-align:-2px;display:inline-block"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg> Edit</button>${(typeof currentRole!=='undefined'&&currentRole==='admin')?`<button class="btn btn-sm" onclick="openCloneModal('${p.id}')" style="border-color:var(--gn);color:var(--gn)" title="Clone WBS ke project baru">⧉ Clone</button>`:''}<button class="btn btn-sm bp" onclick="openModal('updProgress')">↑ Update</button></div>` : `<div style="display:flex;align-items:center"><span style="font-size:10px;font-weight:600;padding:4px 10px;border-radius:8px;background:rgba(245,158,11,.12);color:var(--yw);border:1px solid rgba(245,158,11,.25);display:inline-flex;align-items:center;gap:5px" title="Anda tidak di-assign ke proyek ini — hanya bisa melihat"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="display:inline-block"><rect x="3" y="11" width="18" height="11" rx="2" ry="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg> Read-only</span></div>`}
    </div>
    ...
    <div class="card" style="margin-bottom:9px">
      ...
      ${(()=>{const _cp=_calcProjCurrentPlan(p.id);const _wn=_getProjCurrentWeek(p);const _wlbl=_wn?` <span style="font-size:8px;background:rgba(59,130,246,.12);color:var(--bl);border:1px solid rgba(59,130,246,.25);border-radius:8px;padding:0 5px;font-family:var(--fm);font-weight:700">W${String(_wn).padStart(2,'0')}</span>`:'';return `<div class="pr"><div class="pl">Actual</div><div class="pb"><div class="pf" style="width:${p.actual}%;background:${bc}"></div></div><div class="pn" style="color:var(--gn)">${p.actual}%</div></div><div class="pr"><div class="pl">Plan${_wlbl}</div><div class="pb"><div class="pf" style="width:${_cp}%;background:linear-gradient(90deg,var(--bl),#60a5fa);opacity:.5"></div></div><div class="pn" style="color:var(--bl)">${_cp.toFixed(1)}%</div></div>`;})()}
    ...
    <div class="card" style="margin:14px 0">
      ...
      <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(210px,1fr));gap:26px">
        ...
        <div>
          ...
          ${_pTotal===0?'<div style="font-size:11px;color:var(--mt);padding:8px 0">Belum ada item procurement</div>':`
          <div style="height:10px;border-radius:6px;background:var(--sf2);overflow:hidden;display:flex">
...
var PROC_STATUS_COLOR = {
  'Waiting Approval':'var(--pu)', 'PO Issued':'var(--or)',
